...
public class AncientSkeletonRaid extends SettlementRaidLevelEvent {
    ...
    public GameMessage getApproachMessage(GameMessage settlementName, boolean isFirst) {
        return (GameMessage) new LocalMessage("misc", "raidapproaching", new Object[] {
                ...
                "raidtype", Localization.translate("misc", "ancient_skeleton_raiders")
        });
    }
    ...
    public GameMessage getDefeatedMessage() {
        return (GameMessage) new LocalMessage("misc", "raiddefeated", "raidtype",
                Localization.translate("misc", "ancient_skeleton_raiders"));
    }
    ...
    public AncientSkeletonRaid(ServerSettlementData Data, SettlementRaidOptions Options) 
    {
        ...
        for (int Raiders = 0; Raiders < Size; Raiders++)
        {
            if (Raiders % 8 == 0)
            {
                this.loadouts.add(new SettlementRaidLoadout("ancient_skeleton_summoner_raider"));
            }
            else if (GameRandom.globalRandom.getChance(0.10F))
            {
                this.loadouts.add(new SettlementRaidLoadout("ancient_skeleton_miner_raider"));
            }
            else
            {
