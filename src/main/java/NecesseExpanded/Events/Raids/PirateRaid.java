...
public class PirateRaid extends SettlementRaidLevelEvent {
    ...
    public GameMessage getApproachMessage(GameMessage settlementName, boolean isFirst) {
        return (GameMessage) new LocalMessage("misc", "raidapproaching", new Object[] {
                ...
                "raidtype", Localization.translate("misc", "pirate_raiders")
        });
    }
    ...
    public GameMessage getDefeatedMessage() {
        return (GameMessage) new LocalMessage("misc", "raiddefeated", "raidtype",
                Localization.translate("misc", "pirate_raiders"));
    }
    ...
    public PirateRaid(ServerSettlementData Data, SettlementRaidOptions Options) 
    {
        ...
        for (int Raiders = 0; Raiders < Size; Raiders++)
        {
            if (Raiders % 12 == 0)
            {
                this.loadouts.add(new SettlementRaidLoadout("pirate_grenadier"));
            }
            else if (Raiders % 8 == 0)
            {
                this.loadouts.add(new SettlementRaidLoadout("pirate_sharpshooter"));
            }
            else
            {
                this.loadouts.add(new SettlementRaidLoadout(GameRandom.globalRandom.getOneOf("pirate_brawler", "pirate_gunner")));
            }
        }
    }
}
