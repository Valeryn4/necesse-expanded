...
public class SlimeCaveRaid extends SettlementRaidLevelEvent {
    ...
    public GameMessage getApproachMessage(GameMessage settlementName, boolean isFirst) {
        return (GameMessage) new LocalMessage("misc", "raidapproaching", new Object[] {
                ...
                "raidtype", Localization.translate("misc", "slimecave_raiders")
        });
    }
    ...
    public GameMessage getDefeatedMessage() {
        return (GameMessage) new LocalMessage("misc", "raiddefeated", "raidtype",
                Localization.translate("misc", "slimecave_raiders"));
    }
    ...
    public SlimeCaveRaid() {
    }

    public SlimeCaveRaid(SettlementLevelData Data, SettlementRaidOptions Options) 
    {
        float Size = Data.countTotalSettlers() * 5 * Options.difficultyModifier;
        for (int Raiders = 0; Raiders < Size; Raiders++)
        {
            if (Raiders % 4 == 0)
            {
                this.loadouts.add(new SettlementRaidLoadout("mage_slime_raider"));
            }
            else
            {
                this.loadouts.add(new SettlementRaidLoadout("warrior_slime_raider"));
            }
        }
    }
}
