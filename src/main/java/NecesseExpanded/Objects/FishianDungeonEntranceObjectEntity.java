...
public class FishianDungeonEntranceObjectEntity extends PortalObjectEntity {
    ...
    public FishianDungeonEntranceObjectEntity(Level level, int x, int y) {
    ...
    public void use(Server server, ServerClient client) 
    {
        ...
        this.destinationIdentifier = new LevelIdentifier(currentIdentifier.stringID + "-dungeon" + this.tileX + "x" + this.tileY);
        Function<LevelIdentifier, Level> levelGenerator = identifier -> 
        {
            (getLevel()).childLevels.add(this.destinationIdentifier);
            return (Level)(new FishianDungeonLevel(identifier, server.world.worldEntity));
        };
