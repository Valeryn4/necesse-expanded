...
public class HappinessModifiersPatch {
    ...
    static void onExit(@This HumanMob ThisSettler, @Return(readOnly = false) List<HappinessModifier> list) {
        ...
        if (ThisSettler.lastFoodEaten != null && ThisSettler.lastFoodEaten.quality != null) {
            ...
            } else if (ThisSettler.lastFoodEaten.quality.happinessIncrease == 24)
            {
        ...
        if (ThisSettler.levelSettler != null) {
            HappinessModifier.bedOutsideModifier = new HappinessModifier(-10,
                    (GameMessage) new LocalMessage("settlement", "bedoutside"));
            HappinessModifier.noBedModifier = new HappinessModifier(-20,
                    (GameMessage) new LocalMessage("settlement", "nobed"));
                ...
                if (Room != null) {
                    try {
                        ...
                        if (NumberOfOccupiedBeds > 1)
                            Modifiers.add(new HappinessModifier(-4 * NumberOfOccupiedBeds,
                                    (GameMessage) (new GameMessageBuilder()).append("settlement", "sharingroom")
                                            .append(" (" + (NumberOfOccupiedBeds - 1) + ")")));
                    } catch (Exception ex) {
                    ...
                    if (Room.getRoomProperty("lights") <= 0)
                        Modifiers.add(new HappinessModifier(-3,
                                (GameMessage) (new GameMessageBuilder()).append("settlement", "roommissinglights")));
                    if (Room.getRoomProperty("outsidefloor") > 0)
                        Modifiers.add(new HappinessModifier(-3,
                                (GameMessage) (new GameMessageBuilder()).append("settlement", "roommissingfloor")));
                } else {
