package NecesseExpanded.Items.Weapons;
...
public class RaiderBomb extends SettlerIgnoredThrowToolItem
{
    public RaiderBomb() {
      this.stackSize = 100;
   ...
   public InventoryItem onAttack(Level level, int x, int y, ItemAttackerMob attackerMob, int attackHeight, InventoryItem item, ItemAttackSlot slot, int animAttack, int seed, GNDItemMap mapContent) {
      ...
      IronBombProjectile projectile = new RaiderBombProjectile(attackerMob.x, attackerMob.y, (float)targetPos.x, (float)targetPos.y, this.getThrowingVelocity(item, attackerMob), newRange, this.getAttackDamage(item), attackerMob);
