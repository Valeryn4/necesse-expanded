package NecesseExpanded.Mobs.Raiders.GraveyardRaiders;
...
public class VampireRaider extends ItemAttackerRaiderMob
{
    ...
    public VampireRaider()
    {
        ...
        this.weapon = new InventoryItem("bloodclaw");
        VampireRaider.lootTable = new LootTable
        (
          new LootItemInterface[]
          {
            (LootItemInterface)LootItem.between("coin", getMaxHealth() / 30, getMaxHealth() / 20),
            (LootItemInterface) new ChanceLootItem(0.25F, "phantomdust", GameRandom.globalRandom.getIntBetween(1, 2))
          }
        );
    }
  ...
  public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level,
    ...
    HumanDrawOptions humanOptions = (new HumanDrawOptions(level, MobRegistry.Textures.cryptVampire)).sprite(sprite).dir(dir)
